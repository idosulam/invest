...
const EMPTY_FILTERS: ScanFilters = {
  min_rsi: "", max_rsi: "", above_sma_200: "", sma_20_above_50: "",
  min_adx: "", bb_oversold: false, bb_overbought: false, min_volume_avg: "",
  max_pe: "", min_pe: "", min_market_cap: "", instrument_type: "", sector: "",
};

type SortField = "symbol" | "last_price" | "change_pct" | "volume_avg_20d" | "rsi_14" | "adx_14" | "bb_pct_b" | "pe_ratio" | "market_cap";
...
export default function ScannerPage() {
  ...
  const runScan = useCallback(async () => {
    ...
    if (filters.above_sma_200) body.above_sma_200 = filters.above_sma_200 === "true";
    if (filters.sma_20_above_50) body.sma_20_above_50 = filters.sma_20_above_50 === "true";
    ...
    if (filters.min_volume_avg) body.min_volume_avg = Number(filters.min_volume_avg);
    if (filters.max_pe) body.max_pe = Number(filters.max_pe);
    if (filters.min_pe) body.min_pe = Number(filters.min_pe);
    if (filters.min_market_cap) body.min_market_cap = Number(filters.min_market_cap);
    ...
    try {
      ...
      const res = await fetch(`/api/v1/scanner/run?sort_by=${sortField}&sort_dir=${sortDir}&limit=200`, {
        method: "POST",
        headers: {
              ...
              <div className="space-y-5">
                ...
                <FilterGroup label="RSI (14)">
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => setFilters((f) => ({ ...f, min_rsi: e.target.value }))}
                      className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2"
                    />
                    <input
                      type="number"
                      ...
                      onChange={(e) => setFilters((f) => ({ ...f, max_rsi: e.target.value }))}
                      className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2"
                    />
                  </div>
                </FilterGroup>
                ...
                <FilterGroup label="Trend">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setFilters((f) => ({ ...f, above_sma_200: e.target.checked ? "true" : "" }))
                      }
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) =>
                        setFilters((f) => ({ ...f, sma_20_above_50: e.target.checked ? "true" : "" }))
                      }
                      className="rounded border-surface-200"
                    />
                    SMA 20 above SMA 50
                ...
                <FilterGroup label="ADX (14) — Trend Strength">
                  <input
                    ...
                    onChange={(e) => setFilters((f) => ({ ...f, min_adx: e.target.value }))}
                    className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2"
                  />
                </FilterGroup>
                ...
                <FilterGroup label="Bollinger Bands">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => setFilters((f) => ({ ...f, bb_oversold: e.target.checked }))}
                      className="rounded border-surface-200"
                    />
                    Oversold (%B &lt; 0.2)
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => setFilters((f) => ({ ...f, bb_overbought: e.target.checked }))}
                      className="rounded border-surface-200"
                    />
                    Overbought (%B &gt; 0.8)
                  </label>
                ...
                <FilterGroup label="Avg Volume (20d)">
                  <input
                    ...
                    onChange={(e) => setFilters((f) => ({ ...f, min_volume_avg: e.target.value }))}
                    className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2"
                  />
                </FilterGroup>
            ...
            <div className="flex items-center justify-between px-3 py-2 border-b border-surface-200">
              <div className="flex items-center gap-3">
                <p className="text-sm font-medium text-surface-900">
                  {hasScanned ? `${total} results` : "Run a scan to see results"}
                </p>
                {scanTime > 0 && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-surface-200">
                      <SortHeader field="symbol" label="Symbol" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <th className="text-left py-2 px-2 font-medium text-surface-700">Name</th>
                      <SortHeader field="last_price" label="Price" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <SortHeader field="change_pct" label="Chg%" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <SortHeader field="rsi_14" label="RSI" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <SortHeader field="adx_14" label="ADX" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <SortHeader field="bb_pct_b" label="%B" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <SortHeader field="volume_avg_20d" label="Vol 20d" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <th className="text-left py-2 px-2 font-medium text-surface-700">Trend</th>
                      <SortHeader field="pe_ratio" label="P/E" current={sortField} dir={sortDir} onClick={toggleSort} />
                      <th className="text-right py-2 px-2 font-medium text-surface-700"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.map((r) => (
                      <tr key={r.instrument_id} className="border-b border-surface-100 hover:bg-surface-50">
                        <td className="py-2 px-2">
                          <Link href={`/instruments/${r.instrument_id}`} className="font-semibold text-primary-600 hover:text-primary-700">
                            {r.symbol}
                          </Link>
                        </td>
                        <td className="py-2 px-2 text-surface-900 max-w-[150px] truncate">{r.name}</td>
                        <td className="py-2 px-2 font-mono">{r.last_price?.toFixed(2) ?? "—"}</td>
                        <td className={`py-2 px-2 font-mono ${format.changeColor(r.change_pct ?? 0)}`}>
                          {r.change_pct !== null ? (
                        ...
                        <td className="py-2 px-2 font-mono">{r.adx_14?.toFixed(1) ?? "—"}</td>
                        <td className="py-2 px-2 font-mono">
                          <BBCell value={r.bb_pct_b} />
                        </td>
                        <td className="py-2 px-2 font-mono text-surface-700">
                          {r.volume_avg_20d ? format.compact(r.volume_avg_20d) : "—"}
                        </td>
                        <td className="py-2 px-2">
                          <div className="flex gap-1">
                            {r.above_sma_200 === true && (
                              <span className="px-1.5 py-0.5 text-xs bg-green-50 text-success-600 rounded">↑200</span>
                            )}
                            {r.above_sma_200 === false && (
                              <span className="px-1.5 py-0.5 text-xs bg-red-50 text-danger-600 rounded">↓200</span>
                            )}
                            {r.sma_20_above_50 === true && (
                              <span className="px-1.5 py-0.5 text-xs bg-green-50 text-success-600 rounded">20>50</span>
                            )}
                            {r.sma_20_above_50 === false && (
                              <span className="px-1.5 py-0.5 text-xs bg-red-50 text-danger-600 rounded">20<50</span>
                            )}
                          </div>
                        </td>
                        <td className="py-2 px-2 font-mono text-surface-700">
                        ...
                        <td className="py-2 px-2 text-right">
                          <Link href={`/instruments/${r.instrument_id}`} className="text-primary-600 hover:text-primary-700 text-xs font-medium">
                            Chart →
                          </Link>
                        </td>
                      </tr>
...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-medium text-surface-700 mb-1.5 uppercase tracking-wide">{label}</label>
