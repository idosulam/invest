...
export const charts = {
  getData: (instrumentId: string, params?: {
    timeframe?: string;
    start?: string;
    end?: string;
    indicators?: string;
...
export const watchlists = {
  ...
  create: (name: string) =>
    request<any>("/watchlists", { method: "POST", body: JSON.stringify({ name }) }),
  update: (id: string, data: { name?: string }) =>
    request<any>(`/watchlists/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  delete: (id: string) =>
